...
3. **Audio mute/unmute honoring the runtime sound setting** — blur mutes, focus restores to user's choice, the user's own mute/unmute button always wins over a stuck forced-mute flag, AND any `Pause()` a focus handler applies can be `UnPause()`d by the mute-toggle path too, not just by another focus event.
...
### The Pause/UnPause vs. mute-flag desync trap (the actual root cause behind the symptom above)
...
>
> Some games implement the native/editor focus path (Check 3's "editor/native path") as
> `Pause()`-on-blur / `UnPause()`-on-focus instead of the reference's `entry.source.mute =
> focus ? userMuted : true`. That's acceptable on its own (see Check 3's failure-mode note), but it
> creates a second, *independent* thing controlling whether audio is actually audible: the engine
> playback state (`isPlaying`/paused), separate from the `.mute` flag. If the game's user-facing
> mute/unmute toggle (`ToggleMute`, `SetMuteAll`, etc.) only ever sets `.mute` and never calls
> `UnPause()`, then a source that got `Pause()`d by a focus-loss event stays **silently paused**
> even after the user turns their sound/music setting back on — because flipping `.mute = false`
> does not resume a paused `AudioSource`. Only the *next* actual focus-regain event (whose handler
> calls `UnPause()`) makes it audible again. This produces the exact same "toggling sound/music
> back on doesn't work until I lose focus and regain focus" symptom as the reverse-invariant bug
> above, but from an unrelated cause — **check both** before concluding which one it is.
>
> **Verify:** grep the audio manager for `.Pause()`. For every source it pauses, confirm the mute
> toggle's write path (whatever sets `.mute = false` to make that source audible again) also calls
> `.UnPause()` on it — don't rely on a future focus-regain event to do that job, since the user
> toggling their setting may happen before that event ever fires again, or in a WebView where the
> native `OnApplicationFocus` path is unreliable in the first place (see Check 6's rationale).
...
| 3 | Audio mute/unmute honors user sound setting (both paths + invariant + reverse invariant + Pause/UnPause-vs-mute desync check) | | | |
